...
homekit_characteristic_t enable_swing = HOMEKIT_CHARACTERISTIC_(CUSTOM_SWING, true, .getter=enable_swing_get, .setter=enable_swing_set);
...
void save_settings() {
	...
	status = sysparam_set_bool(SWING_SYSPARAM, enable_swing.value.bool_value);
	if (status != SYSPARAM_OK) {
		flash_error = status;
	}
...
void settings_init() {
	...
	if (status == SYSPARAM_OK) {
		...
		status = sysparam_set_int8(FAN_SYSPARAM, 0);
		if (status != SYSPARAM_OK) {
			flash_error = status;
		}
	}

	status = sysparam_get_bool(SWING_SYSPARAM, &bool_value);
	if (status == SYSPARAM_OK) {
		enable_swing.value.bool_value = bool_value;
	} else {
		status = sysparam_set_bool(SWING_SYSPARAM, true);
		if (status != SYSPARAM_OK) {
			flash_error = status;
		}
	}
...
homekit_characteristic_t revision = HOMEKIT_CHARACTERISTIC_(FIRMWARE_REVISION, "1.2.2");
